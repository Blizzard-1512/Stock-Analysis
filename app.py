...
def display_portfolio_dashboard():
    ...
    # Initialize session state for portfolio data if not already present
    if 'portfolio_data' not in st.session_state:
        st.session_state.portfolio_data = []
    
    # Form for adding stocks to the portfolio
    with st.form(key="stock_input_form"):
        ...
        if submit_button and symbol:
            ...
            st.success(f"Added {shares} shares of {symbol.upper()} to your portfolio.")
    ...
    # Button to analyze the portfolio
    if st.button("Analyze Portfolio"):
        if st.session_state.portfolio_data:
            st.markdown("### Portfolio Performance Metrics")
            
            # Fetch historical data for each stock in the portfolio
            portfolio_symbols = [stock['symbol'] for stock in st.session_state.portfolio_data]
            portfolio_data = yf.download(portfolio_symbols, period="1y")
            ...
            if not portfolio_data.empty:
                ...
                portfolio_value = portfolio_returns.copy()
                for stock in st.session_state.portfolio_data:
                    portfolio_value[stock['symbol']] *= stock['shares']
                
                # Calculate portfolio total value
                portfolio_value['Total'] = portfolio_value.sum(axis=1)
                
                # Calculate portfolio return and standard deviation
                portfolio_return = portfolio_value['Total'].mean() * 252  # Annualized return
                portfolio_std_dev = portfolio_value['Total'].std() * (252 ** 0.5)  # Annualized standard deviation
                ...
                metrics_data = {
                    ...
                    "Value": [
                        portfolio_return * 100,  # Convert to percentage
                        portfolio_std_dev * 100  # Convert to percentage
                    ]
                }
